...
class _BNode(object):
    ...
    def __repr__(self):
        name = getattr(self, "children", 0) and "Ramificação" or "Folha"
...
class BTree(object):
    ...
    # Metodo construtor onde eh definida a raiz e a ordem da arvore
    def __init__(self, order):
        self.order = order
        self._root = self._bottom = self.LEAF(self)

    # Funcao que encontra um caminho ate determinada folha da arvore
    ...
    # Funcao que verificar se um item esta presente em uma das ramificacoes
    def _present(self, item, ancestors):
        last, index = ancestors[-1]
        return index < len(last.contents) and last.contents[index] == item

    # Metodo de insercao de um item na arvore
    ...
    # Metodo de remocao de um item na arvore
    ...
    # Metodo para determinar se determinado item esta presente ou nao na arvore
    def __contains__(self, item):
        return self._present(item, self._path_to(item))

    # Metodo de iteracao pela arvore
    def __iter__(self):
        def _recurse(node):
            if node.children:
